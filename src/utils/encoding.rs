...
impl Encoder {
    ...
    pub fn encode_mod_string(&self, data: &[u16]) -> String {
        ...
        self.logger.log_default(&format!("Encoded mod string: {}", encoded));
        encoded
    }
    ...
    pub fn decode_mod_string(&self, data: String) -> Result<Vec<u16>, EncodingError> {
        ...
        let decoded_data = decoded.split("&").map(|x| x.parse::<u16>().unwrap()).collect();
        self.logger.log_default(&format!("Decoded mod string: {:?}", decoded_data));
        Ok(decoded_data)
    }
    ...
    fn create_mod_string(&self, data: &[u16]) -> String {
        ...
        let mod_string = data.iter().map(|x| x.to_string()).collect::<Vec<String>>().join("&");
        self.logger.log_default(&format!("Created mod string: {}", mod_string));
        mod_string
    }
}
